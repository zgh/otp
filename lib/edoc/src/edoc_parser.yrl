...
Nonterminals
start spec func_types func_type utype_list utype_tuple utypes utype
ptypes ptype nutype function_name where_defs defs defs2 def typedef etype
throws qname ref aref mref lref pref var_list vars fields field
futype_list bin_base_type bin_unit_type guard guards.
...
spec -> func_types:
    #t_spec{clauses = lists:reverse('$1')}.
spec -> function_name func_types:
    #t_spec{name = '$1', clauses = lists:reverse('$2')}.

function_name -> atom: #t_name{name = tok_val('$1')}.

%% Produced in reverse order.
func_types -> func_type: ['$1'].
func_types -> func_types ';' func_type : ['$3' | '$1'].

func_type -> utype_list '->' utype 'when' guards where_defs :
    #t_clause{type = 
                  #t_fun{args = element(1, '$1'), range = '$3', guards = '$5'},
              defs = lists:reverse('$6')}.
func_type -> utype_list '->' utype where_defs:
    #t_clause{type = #t_fun{args = element(1, '$1'), range = '$3'},
              defs = lists:reverse('$4')}.
...
guard -> atom '(' utypes ')' : #t_guard{name = #t_name{name = tok_val('$1')},
                                        args = lists:reverse('$3')}.
...
ptype -> atom utype_list:
             case {tok_val('$1'), element(1, '$2')} of
                 {nil, []} ->
                     ...
                     %% compatibility with Dialyzer, which does not
                     %% separate '[]' from 'nil()'.
                     #t_nil{};
                 {list, [T]} ->
                 ...
                 {'fun', [#t_fun{}=Fun]} ->
                     ...
                     %% Due to compatibility with Dialyzer.
                     Fun;
                 {'fun', []} ->
