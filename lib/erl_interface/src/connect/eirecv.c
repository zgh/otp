...
int 
ei_decode_header_internal(const char *buf, int *index, erlang_msg *msg)
{
  char *s;
  int arity;
  int version;
  ...
  if ((get8(s) != ERL_PASS_THROUGH)
      ...
      || ei_decode_long(buf,index,&msg->msgtype))
  {
  ...
  case ERL_SEND:          /* { SEND, Cookie, ToPid } */
    ...
    if (ei_decode_atom(buf,index,msg->cookie)
	|| ei_decode_pid(buf,index,&msg->to))
    {
	erl_errno = EIO;
	return -1;
    }
  ...
  case ERL_REG_SEND:     /* { REG_SEND, From, Cookie, ToName } */
    if (ei_tracelevel > 0) show_this_msg = 1;
    if (ei_decode_pid(buf,index,&msg->from)
	|| ei_decode_atom(buf,index,msg->cookie)
	|| ei_decode_atom(buf,index,msg->toname))
    {
	erl_errno = EIO;
	return -1;
    }

    /* actual message is remaining part of bufbuf, plus any unread bytes */
    break;
  ...
  case ERL_SEND_TT:      /* { SEND_TT, Cookie, ToPid, TraceToken } */
    ...
    if (ei_decode_atom(buf,index,msg->cookie)
	|| ei_decode_pid(buf,index,&msg->to)
	|| ei_decode_trace(buf,index,&msg->token))
    {
	erl_errno = EIO;
	return -1;
    }
  ...
  return show_this_msg;
}

int
ei_decode_header(const char *buf, int *index, erlang_msg *msg)
{
  if (ei_decode_header_internal(buf, index, msg) == -1) {
    return -1;
  }
		  ...
		  int staticbufp, unsigned ms)
{
  ...
  /* now decode header */
  if ((show_this_msg = ei_decode_header_internal(header, &index, msg)) == -1)
    return -1;
